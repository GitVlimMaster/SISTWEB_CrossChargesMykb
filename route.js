 ...
 */
 ...
 APP.use(function (req, res, next) {
     ...
     res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline' cdnjs.cloudflare.com cdn.jsdelivr.net unpkg.com cdn.datatables.net code.jquery.com; style-src 'self' cdn.jsdelivr.net fonts.googleapis.com 'unsafe-inline'; img-src 'self' data: blob: cdnjs.cloudflare.com cdn.jsdelivr.net unpkg.com *.gstatic.com; font-src 'self' cdnjs.cloudflare.com cdn.jsdelivr.net unpkg.com fonts.googleapis.com fonts.gstatic.com; object-src 'none'; frame-src 'none'");
 ...
 APP.post("/inquiry", async (req, res) => {
     ...
     try {
        ...
        const transCode = await getTransactionCode(req.body.hotel, req.session.hotelInformation.hotelsData, req.session.camaleon);
        // req.session.hotelInformation.hotelsData.opera_id_center = await getTransactionCode(req.body.hotel, req.session.hotelInformation.hotelsData);
        req.session.hotelInformation.hotelsData.opera_id_center = transCode;
        const response = await CONNECTTOAPI.Post("getHotelById", { id_hotel: req.body.hotel })
        if (response.hotelsData.status != "OK" ) {
             ...
             WORKER_MAIN.postMessage({connectinfo: { port: response.hotelsData.port, host: response.hotelsData.ip }, xmlpos: { linkstart: linkstart, description: description, postinquiry: inquiry }});
         } // If
         else {
 ...
 async function getTransactionCode(hotel, actual_hotel, type) {
    try {
        return new Promise((resolve, reject) => {
            if(type) {
                CONNECTTOAPI.Post("getCodeExperiences", {"center": actual_hotel.id_hotel, "id_hotel": parseInt(hotel)}).then(result => {
                    if(result.status == 200) {
                        resolve(result.message.transaction_code);
                    }
                    else {
                        resolve(actual_hotel.opera_id_center);
                    } 
                });
            }
            else {
         ...
         CONNECTTOAPI.Post("getLastCheckNumber", { id_hotel_b: req.body.hotelB }).then((consecutivenumber) => {
             ...
             const CHARGEINFO = {
                 ...
                 postrequest: XMLGENERATE.IFC8("PostRequest", { 
                     ...
                     tip: "0", 
                     totalamount: AMOUNT, 
                     waiterid: "CrossCharges " + req.body.category, 
             ...
             console.log(CHARGEINFO.postrequest);
             ...
             ACTUALPROCESS.PostrequestProcess(CHARGEINFO).then((IFC8RESPONSE) => { // Enviamos los datos necesarios que nesecita opra para realizar un cargo mediante un middleware
                if(!IFC8RESPONSE.PostAnswer){
                    console.log(IFC8RESPONSE)
                ...
                CREATEDOCUMENT.SaveIMG(CROSSCHARGE.ticket, "./public/ticket/", "ticket_").then((ticket_name) => {
                    ...
                    const CHARGEINFO = {
                        ...
                        complex: CROSSCHARGE.complex,
                        dolar_change: CROSSCHARGE.dolar_change, 
                        folio: FOLIO, 
                        ...
                        no_ticket: CROSSCHARGE.no_ticket,
 ...
 APP.post("/gethotel", (req, res) => {
    ...
    var contain_tmp_hotels = req.session.allHotels.hotelsData;
     ...
     if ((username.length != 0) && (usr_password.length != 0)) {
         ...
         CONNECTTOAPI.Post("login", { "mac": "none", "user": username, "password": encrypted }).then((resolve) => {
             ...
             if (logeddUser.status == "success") {
                 CONNECTTOAPI.Post("getHotelById", {id_hotel: logeddUser.userData.id_hotel}).then(async (hotelData)=>{
                     req.session.hotelsession = hotelData.hotelsData;
                     req.session.camaleon = (hotelData.hotelsData.opera_id_center == 0) ? true : false;
                     ...
                     if (logeddUser.length != 0) {
                        req.session.allHotels = await CONNECTTOAPI.Post("getCatHotels", {"complex": logeddUser.userData.complex});
                        res.send('200');
                     } // If
  ...
  */
  APP.post("/getHuespedLastname", (req, res) => {
     try {
         CONNECTTOAPI.Post("getHotelById", { id_hotel: req.body.hotelToSearch}).then(async getHotelInfo => {
            const transCode = await getTransactionCode(req.body.hotelToSearch, req.session.hotelInformation.hotelsData, req.session.camaleon);
             //const transCode = await getTransactionCode(getHotelInfo.hotelsData.id_hotel, getHotelInfo.hotelsData, req.session.camaleon);
  ...
  */
  APP.get('/', function (req, res) {
    if (req.session.loggedin) {
        var tmp_hotels_for_count = (req.session.allHotels.status != "error") ? req.session.allHotels.hotelsData : [];
        var notLoggedHotel = [];
        ...
        CONNECTTOAPI.Post("getHotelById", { "id_hotel": req.session.hotelid }).then((resolve) => {
            ...
            let view = (req.session.user.complex == 2) ? "mandarina" : "index";
 ...
 function getConsumeCenter(hotel) {
    return new Promise((resolve, reject) => {
        CONNECTTOAPI.Post("/getConsumeCenters", { hotel: hotel }).then((response) => {
            resolve(response);
        });
    });
}
  ...
  * @returns {void}
  */
 APP.get("/user/:id/:hotel", async (req, res) => {
    if (req.session.loggedin) {
        var contain_hotel_request = [];
        let consume = await getConsumeCenter(req.session.hotelid);
        console.log(">>> Consumer Center:", req.session.hotelInformation);
        CONNECTTOAPI.Post("getHotelById", { id_hotel: req.params.hotel }).then((resolve) => {
            contain_hotel_request = resolve;
            let view = (req.session.user.complex == 2) ? "user_mandarina" : "user";
            let hotel = req.session.user.id_hotel;
            res.render(view, {
                isExperiencias: (hotel == 31 || hotel == 30 || hotel == 29 || hotel == 6) ? 1 : 0,
                ...
                initalValue: consume.message[0].name,
                consumesCenters: consume.message,
                countConsumesCenters: (consume.message.length > 0) ? true : false,
